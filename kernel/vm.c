...
// create kpagetable for each process, identical to kernel_pagetable
...
// free kpagetable
// assumming kstack is unmaped and freed
void kvm_freepagetable(pagetable_t pagetable) {
  // unmap leaves
  uint64 page_size;
  ...
  uvmunmap(pagetable, KERNBASE, page_size/PGSIZE, 0);

  page_size = PGROUNDUP(PHYSTOP-(uint64)etext);
  uvmunmap(pagetable, (uint64)etext, page_size/PGSIZE, 0);
...
void vmprint(pagetable_t pagetable) {
  ...
  for(int i = 0; i < 512; i++) {
    ...
    if(pte & PTE_V) {
      printf("..%d: pte %p pa %p\n", i, pte, PTE2PA(pte));
      pagetable_t pagetable_l1 = (pagetable_t)PTE2PA(pte);

      for(int j = 0; j< 512; j++) {
        pte_t pte = pagetable_l1[j];
        if(pte & PTE_V) {
          printf(".. ..%d: pte %p pa %p\n", j, pte, PTE2PA(pte));
          pagetable_t pagetable_l0 = (pagetable_t)PTE2PA(pte);
          
          for(int k = 0; k < 512; k++) {
            pte_t pte = pagetable_l0[k];
            if(pte & PTE_V)
              printf(".. .. ..%d: pte %p pa %p\n", k, pte, PTE2PA(pte));
          }
        }
